...
combineInstructionsOverFunction(Function &F, InstCombineWorklist &Worklist,
    ...
    // Module Construction
    Module* mod = new Module("delta", F.getContext());
    Constant* c = mod->getOrInsertFunction("delta",
                                           Type::getDoubleTy(F.getContext()),
                                           Type::getDoubleTy(F.getContext()));
    Function* delta_f = cast<Function>(c);
    delta_f->setCallingConv(CallingConv::C);
    
    BasicBlock *deltaBlock = BasicBlock::Create(delta_f -> getContext(), "delta", delta_f);
