...
static cl::opt<bool>
    EnableIfConversion("stable-enable-if-conversion", cl::init(true), cl::Hidden,
                       cl::desc("Enable if-conversion during vectorization."));

/// Loops with a known constant trip count below this number are vectorized only
/// if no scalar iteration overheads are incurred.
static cl::opt<unsigned> TinyTripCountVectorThreshold(
    "stable-vectorizer-min-trip-count", cl::init(16), cl::Hidden,
    cl::desc("Loops with a constant trip count that is smaller than this "
             "value are vectorized only if no scalar iteration overheads "
             "are incurred."));

static cl::opt<bool> MaximizeBandwidth(
    "stable-vectorizer-maximize-bandwidth", cl::init(false), cl::Hidden,
    cl::desc("Maximize bandwidth when selecting vectorization factor which "
             "will be determined by the smallest type in loop."));

static cl::opt<bool> EnableInterleavedMemAccesses(
    "stable-enable-interleaved-mem-accesses", cl::init(false), cl::Hidden,
    cl::desc("Enable vectorization on interleaved memory accesses in a loop"));

/// Maximum factor for an interleaved memory access.
static cl::opt<unsigned> MaxInterleaveGroupFactor(
    "stable-max-interleave-group-factor", cl::Hidden,
...
static cl::opt<unsigned> ForceTargetNumScalarRegs(
    "stable-force-target-num-scalar-regs", cl::init(0), cl::Hidden,
    cl::desc("A flag that overrides the target's number of scalar registers."));

static cl::opt<unsigned> ForceTargetNumVectorRegs(
    "stable-force-target-num-vector-regs", cl::init(0), cl::Hidden,
...
static cl::opt<unsigned> ForceTargetMaxScalarInterleaveFactor(
    "stable-force-target-max-scalar-interleave", cl::init(0), cl::Hidden,
    cl::desc("A flag that overrides the target's max interleave factor for "
             "scalar loops."));

static cl::opt<unsigned> ForceTargetMaxVectorInterleaveFactor(
    "stable-force-target-max-vector-interleave", cl::init(0), cl::Hidden,
    cl::desc("A flag that overrides the target's max interleave factor for "
             "vectorized loops."));

static cl::opt<unsigned> ForceTargetInstructionCost(
    "stable-force-target-instruction-cost", cl::init(0), cl::Hidden,
    cl::desc("A flag that overrides the target's expected cost for "
             "an instruction to a single constant value. Mostly "
             "useful for getting consistent testing."));

static cl::opt<unsigned> SmallLoopCost(
    "stable-small-loop-cost", cl::init(20), cl::Hidden,
    cl::desc(
        "The cost of a loop that is considered 'small' by the interleaver."));

static cl::opt<bool> LoopVectorizeWithBlockFrequency(
    "stable-loop-vectorize-with-block-frequency", cl::init(false), cl::Hidden,
...
static cl::opt<bool> EnableLoadStoreRuntimeInterleave(
    "stable-enable-loadstore-runtime-interleave", cl::init(true), cl::Hidden,
    cl::desc(
        "Enable runtime interleaving until load/store ports are saturated"));

/// The number of stores in a loop that are allowed to need predication.
static cl::opt<unsigned> NumberOfStoresToPredicate(
    "stable-vectorize-num-stores-pred", cl::init(1), cl::Hidden,
    cl::desc("Max number of stores to be predicated behind an if."));

static cl::opt<bool> EnableIndVarRegisterHeur(
    "stable-enable-ind-var-reg-heur", cl::init(true), cl::Hidden,
    cl::desc("Count the induction variable only once when interleaving"));

static cl::opt<bool> EnableCondStoresVectorization(
    "stable-enable-cond-stores-vec", cl::init(true), cl::Hidden,
    cl::desc("Enable if predication of stores during vectorization."));

static cl::opt<unsigned> MaxNestedScalarReductionIC(
    "stable-max-nested-scalar-reduction-interleave", cl::init(2), cl::Hidden,
    cl::desc("The maximum interleave count to use when interleaving a scalar "
             "reduction in a nested loop."));

static cl::opt<unsigned> PragmaVectorizeMemoryCheckThreshold(
    "stable-pragma-vectorize-memory-check-threshold", cl::init(128), cl::Hidden,
    cl::desc("The maximum allowed number of runtime memory checks with a "
             "vectorize(enable) pragma."));

static cl::opt<unsigned> VectorizeSCEVCheckThreshold(
    "stable-vectorize-scev-check-threshold", cl::init(16), cl::Hidden,
    cl::desc("The maximum number of SCEV checks allowed."));

static cl::opt<unsigned> PragmaVectorizeSCEVCheckThreshold(
    "stable-pragma-vectorize-scev-check-threshold", cl::init(128), cl::Hidden,
...
void InnerLoopVectorizer::fixReduction(PHINode *Phi) {
  ...
  for (unsigned Part = 0; Part < UF; ++Part) {
    ...
    /**************************** Add metadata*/
    if(RK == RecurrenceDescriptor::RK_FloatAdd || RK == RecurrenceDescriptor::RK_FloatMult){
        StringRef reduction_type = RK == RecurrenceDescriptor::RK_FloatAdd ? "add" : "mult";
        MDNode* N = MDNode::get(Latch->getContext(), MDString::get(Latch->getContext(), reduction_type));
        cast<PHINode>(VecRdxPhi) -> setMetadata("reduction", N);
    }
    /**************************** End metadata*/
...
char raic::LoopVectorize::ID = 0;
static RegisterPass<raic::LoopVectorize> X("loop_vectorize", "Loop Vectorize Pass");
...
Pass *createLoopVectorizePass(bool NoUnrolling, bool AlwaysVectorize) {
    return new raic::LoopVectorize(NoUnrolling, AlwaysVectorize);
}
}
...
bool raic::LoopVectorizePass::processLoop(Loop *L) {
...
bool raic::LoopVectorizePass::runImpl(
...
PreservedAnalyses raic::LoopVectorizePass::run(Function &F,
