...
namespace {
    ...
    public:
        ...
        Value *simplify(Instruction *FAdd, PerturbationMap &perturbation_map, IRBuilder<> &delta_builder);
        
    private:
        typedef SmallVector<const FAddend*, 4> AddendVect;
        
        Value *simplifyFAdd(AddendVect& V, unsigned InstrQuota, Perturbation &p , Value *Inst, IRBuilder<> &delta_builder);
        
        Value *performFactorization(Instruction *I, PerturbationMap &perturbation_map, IRBuilder<> &delta_builder);
...
//   (y / x) +/- (z / x)               (y +/- z) / x
//
Value *FAddCombine::performFactorization(Instruction *I, PerturbationMap &perturbation_map, IRBuilder<> &delta_builder) {
    ...
    //***** add perturbation to RI ****
    
    // check I0, I1 have existing perturbation
    auto iter_I0 = perturbation_map.find(I0);
    auto iter_I1 = perturbation_map.find(I1);
    Perturbation p;
    if(iter_I0 != perturbation_map.end()){
        p.compositionAdd(iter_I0 -> second, delta_builder);
    }

    if(iter_I1 != perturbation_map.end()){
        p.compositionAdd(iter_I1 -> second, delta_builder);
    }
    
    //check NewAddSub is ConstantFP
    if(ConstantFP *CFP = dyn_cast<ConstantFP>(NewAddSub)) {
        p.insertConstFactor(Factor, cast<ConstantFP>(AddSub0), cast<ConstantFP>(AddSub1),
                            isMpy ? Instruction::FMul : Instruction::FDiv,
                            static_cast<Instruction::BinaryOps>(I->getOpcode()), I, delta_builder);
    } else { //NewAddSub is not ConstantFP
        p.insertFactor(Factor, AddSub1, isMpy ? Instruction::FMul : Instruction::FDiv, I, delta_builder);
    }
    ...
    if (isMpy) {
        ...
        //***insert perturbation to perturbation map
        perturbation_map[RI] = p;
        //*****************************************
...
Value *FAddCombine::simplify(Instruction *I, PerturbationMap &perturbation_map, IRBuilder<> &delta_builder) {
    ...
    //Record new perturbation introduced in FAdd;
    Perturbation p;
    
    unsigned OpndNum = FAddend::drillValueDownOneStep(I, Opnd0, Opnd1);
    //************** Collecting existing perturbation in I
    if(perturbation_map.find(I) != perturbation_map.end()){
        p.compositionAdd(perturbation_map[I], delta_builder);
    }
    //****************************************************
    ...
    if (!Opnd0.isConstant()){
        ...
        if(perturbation_map.find(I->getOperand(0)) != perturbation_map.end()){
            p.compositionAdd(perturbation_map[I->getOperand(0)], delta_builder);
        }
    }
    ...
    if (OpndNum == 2 && !Opnd1.isConstant()){
        ...
        if(perturbation_map.find(I->getOperand(1)) != perturbation_map.end()){
            p.compositionAdd(perturbation_map[I->getOperand(1)], delta_builder);
        }
    }
    ...
    if (Opnd0_ExpNum && Opnd1_ExpNum) {
        ...
        if (Value *R = simplifyFAdd(AllOpnds, InstQuota, p, I, delta_builder)){
            perturbation_map[R] = p;
            return R;
        }
    }
    ...
    if (Opnd1_ExpNum) {
        ...
        if (Value *R = simplifyFAdd(AllOpnds, 1, p, I, delta_builder)){
            perturbation_map[R] = p;
            return R;
        }
    }
    ...
    return performFactorization(I, perturbation_map, delta_builder);
}

Value *FAddCombine::simplifyFAdd(AddendVect& Addends, unsigned InstrQuota, Perturbation &p, Value *Inst, IRBuilder<> &delta_builder) {
    unsigned AddendNum = Addends.size();
    assert(AddendNum <= 4 && "Too many addends");
    
    //**************perturbation
    int constantIndex; //because constant is inserted last, we have to consider asso/comm
    //***********************
    ...
    for (unsigned SymIdx = 0; SymIdx < AddendNum; SymIdx++) {
        //******* AddendVect for commutivity perturbation
        AddendVect CommVect;
        ...
        for (unsigned SameSymIdx = SymIdx + 1;
            ...
            if (T && T->getSymVal() == Val) {
                ...
                //*************Attach Commutivity Perturbation ****************
                for(auto it : CommVect){
                    Value *Y, *Z;
                    if(it -> getSymVal() != nullptr){
                        Y = delta_builder.CreateFMul(it->getSymVal(), it->getCoef().getValue(Instr->getType()));
                    } else {
                        Y = it->getCoef().getValue(Instr->getType());
                    }
                    
                    if(T -> getSymVal() != nullptr){
                        Z = delta_builder.CreateFMul(T->getSymVal(), T->getCoef().getValue(Instr->getType()));
                    } else {
                        Z = T->getCoef().getValue(Instr->getType());
                    }
                    
                    p.insertCommAndAsso(Y, Z, Inst, delta_builder);
                }
                //*************************************************************
            } else {
                if(T != nullptr){
                    CommVect.push_back(T);
                }
            }
        }
        ...
        if (StartIdx + 1 != SimpVect.size()) {
            ...
            for (unsigned Idx = StartIdx + 1; Idx < SimpVect.size(); Idx++){
                ...
                //*** Attach Association Perturbation into R *******
                if(SymIdx != 0){
                    // Addend vector for association perturbation
                    AddendVect AssoVect = SimpVect;
                    AssoVect.resize(StartIdx);
                    Value *Y,*Z;
                    
                    // Iterate the addends, creating fadd/fsub using adjacent two addends.
                    Value *LastVal = nullptr;
                    
                    for (const FAddend *Opnd : AssoVect) {
                        Value *V;
                        
                        const FAddendCoef &Coeff = Opnd -> getCoef();
                        
                        if (Opnd ->isConstant()) {
                            V = Coeff.getValue(Instr->getType());
                        }else{
                            Value *OpndVal = Opnd -> getSymVal();
                            V = delta_builder.CreateFMul(OpndVal, Coeff.getValue(Instr->getType()));
                        }
                        ...
                        LastVal = delta_builder.CreateFAdd(LastVal, V);
                    }
                    
                    Y = LastVal;
                    
                    if(Val){
                        Z = delta_builder.CreateFMul(Val, SimpVect[Idx]->getCoef().getValue(Instr->getType()));
                        p.insertCommAndAsso(Y, Z, Inst, delta_builder);
                        p.insertConstFactor(Val, cast<ConstantFP>(R.getCoef().getValue(Instr->getType())),
                                            cast<ConstantFP>((*SimpVect[Idx]).getCoef().getValue(Instr->getType())),
                                            Instruction::FMul, Instruction::FAdd, Inst, delta_builder);
                    } else {
                        Z = SimpVect[Idx]->getCoef().getValue(Instr->getType());
                        p.insertConstCommAndAsso(Y, cast<ConstantFP>(R.getCoef().getValue(Instr->getType())),
                                                 cast<ConstantFP>(Z), Inst, delta_builder);
                    }
                }else{ //SymIdx == 0
                    if(Val){
                        p.insertConstFactor(Val, cast<ConstantFP>(R.getCoef().getValue(Instr->getType())),
    ...
    if (ConstAdd){
        //***********perturbation for constant
        if(constantIndex != 0 && constantIndex != SimpVect.size()){
            for(int i = constantIndex; i< SimpVect.size(); i ++){
                Value *Y = delta_builder.CreateFMul(SimpVect[i]->getSymVal(), SimpVect[i]->getCoef().getValue(Instr->getType()));
                Value *Z = ConstAdd->getCoef().getValue(Instr->getType());
                p.insertCommAndAsso(Y, Z, Inst, delta_builder);
            }
        }
...
Instruction *InstCombiner::visitFAdd(BinaryOperator &I) {
    ...
    if (I.hasUnsafeAlgebra()) {
        if (Value *V = FAddCombine(Builder).simplify(&I, perturbation_map_, delta_builder_))
            return replaceInstUsesWith(I, V);
    }
